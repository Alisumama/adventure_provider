...
class _TrackDetailScreenState extends State<TrackDetailScreen>
          ...
          child: Column(
            ...
            children: [
              ListTile(
                ...
                title: Text(
                  'Choose from gallery',
                  style: GoogleFonts.poppins(color: AppColors.surface),
                ),
                onTap: () async {
                  Get.back<void>();
  ...
  Future<void> _confirmDeleteTrackPhoto(
    TrackController c,
    int photoIndex,
  ) async {
    final ok = await Get.dialog<bool>(
                    ...
                    markers: [
                      for (var i = 0; i < track.flags.length; i++)
                        _buildFlagMarker(
                          track.flags[i],
                          i,
                          () => _onFlagTap(context, track.flags[i]),
                        ),
                    ],
                  ),
                ],
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () {
                                Get.snackbar(
                                  'Coming soon',
                                  'Track following will be available in a future update.',
                                  snackPosition: SnackPosition.BOTTOM,
                                  backgroundColor: AppColors.darkSurface,
    ...
    return Wrap(
      ...
      children: [
        _ActionChip(
          ...
          label: '${track.likesCount}',
          onTap: () => c.toggleLike(tid),
        ),
        _ActionChip(
          icon: track.isSaved
              ? Icons.bookmark_rounded
              : Icons.bookmark_border_rounded,
          label: '${track.savesCount}',
          onTap: () => c.toggleSave(tid),
        ),
      ],
    );
  }
                  ...
                  children: [
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 20),
                          _badge(
                            _formatKindLabel(track.difficulty),
                            _difficultyBadgeColor(track.difficulty),
                          ),
                          const SizedBox(width: 12),
                          _badge(
                            _formatKindLabel(track.type),
                    ...
                    Padding(
                      ...
                      child: Obx(() {
                        ...
                        final childCount = photos.length + (isOwner ? 1 : 0);
                        return GridView.builder(
                          ...
                          itemBuilder: (context, i) {
                            if (isOwner && i == photos.length) {
                              return _AddTrackPhotoTile(
                                onTap: () => _openTrackPhotoSourceSheet(c),
                              );
                            }
                            final url = photos[i];
                            final resolved =
                                ApiConfig.resolveMediaUrl(url) ?? url;
                            return _TrackPhotoGridTile(
                              imageUrl: resolved,
                              onPhotoTap: () =>
                                  openTrackPhotoFullscreen(context, url),
                              onDelete: isOwner
                                  ? () => _confirmDeleteTrackPhoto(c, i)
                                  : null,
                            );
                          },
                        );
...
class _AddTrackPhotoTile extends StatelessWidget {
  const _AddTrackPhotoTile({required this.onTap});
